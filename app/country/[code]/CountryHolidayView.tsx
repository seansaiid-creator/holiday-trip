...
function getDisplayName(
  ...
  if (cat === 'substitute' || h.is_substitute) {
    ...
    if (originalName) {
      return mode === 'short' ? `${originalName} sub.` : `${originalName} substitute`;
    }
...
export default function CountryHolidayView({
  country, holidays, travelTips, meta,
  priceItems, countryPrices, currencyCode, currencySymbol,
}: Props) {
  ...
  const calendarCells = useMemo(() => {
    ...
    for (let i = 0; i < startWeekday; i++) cells.push({ day: null, date: null, weekday: i });
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-5">
          ...
          <div className="grid grid-cols-7 gap-1">
            {calendarCells.map((cell, idx) => {
              ...
              const tooltipName = primary ? getDisplayName(primary, 'full', holidayById) : '';
                  ...
                  {cell.day !== null && (
                    <>
                      <div className={`text-xs font-semibold leading-none mb-0.5 ${
                        hasHoliday ? 'text-red-700' : isSunday ? 'text-red-600' : 'text-gray-700'
                      }`}>
                        {cell.day}
                      </div>
                      {primary && (
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="flex-1 px-3 py-3 min-w-0 flex gap-2 items-start">
                        {/* Name area — takes remaining space */}
                        ...
                        {/* Badges — fixed right column, stacked vertically */}
                        ...
                        {tip ? (
                          ...
                          <>
                            ...
                            {!subExplanation && !showRawNote && (
                              <p className="text-gray-500 italic text-[13px]">No additional details yet.</p>
                            )}
                          </>
                        )}
  ...
  return (
    <>
      ...
      {holiday.manual_locked && (
        <span title="Manually locked — protected from automated overwrites" className="text-[11px] text-gray-400">
          🔒
        </span>
      )}
    </>
...
function getVerificationBadge(status: string | null): { label: string; color: string; title: string } | null {
  ...
  switch (status) {
    ...
    case 'single_source': return { label: 'Tentative', color: 'bg-amber-50 text-amber-700 border-amber-200', title: 'Tentative — based on a single source. Confirm with an official source before booking.' };
    case 'pending': return { label: 'Unverified', color: 'bg-gray-50 text-gray-600 border-gray-200', title: 'Unverified — not yet cross-checked against multiple sources.' };
    default: return null;
  }
}
...
function TravelTipBody({ tip }: { tip: TravelTip }) {
  return (
    <div className="space-y-3">
      {tip.what_is_it && (
        <div>
          <div className="text-[11px] uppercase tracking-wide text-gray-500 font-semibold mb-1">What it is</div>
          <p className="text-sm text-gray-700">{tip.what_is_it}</p>
        </div>
      )}
      {tip.traveler_impact && (
        <div>
          <div className="text-[11px] uppercase tracking-wide text-gray-500 font-semibold mb-1">Traveler impact</div>
          <p className="text-sm text-gray-700">{tip.traveler_impact}</p>
        </div>
      )}
      {tip.cautions && (
        <div>
          <div className="text-[11px] uppercase tracking-wide text-gray-500 font-semibold mb-1">Cautions</div>
          <p className="text-sm text-gray-700">{tip.cautions}</p>
        </div>
      )}
      {tip.recommendations && (
        <div>
          <div className="text-[11px] uppercase tracking-wide text-gray-500 font-semibold mb-1">Recommendations</div>
      ...
      {tip.tips && (
        <div>
          <div className="text-[11px] uppercase tracking-wide text-gray-500 font-semibold mb-1">Tips</div>
...
const PRICE_ICONS: Record<string, string> = {
  meal_local: '🍜', bigmac: '🍔', starbucks_latte: '☕',
  beer_bar: '🍺', beer_convenience: '🍺',
  transit_single: '🚇', taxi_start: '🚕', airport_city: '✈️',
  water_convenience: '💧', water_supermarket: '💧',
  coke_convenience: '🥤', coke_supermarket: '🥤',
        ...
        {CATEGORIES.map((cat) => (
          <button
            ...
            className={`text-xs font-medium px-4 py-1.5 rounded-full border transition-colors ${
              activeTab === cat.key ? 'bg-gray-900 text-white border-gray-900' : 'bg-white text-gray-600 border-gray-300 hover:bg-gray-50'
            }`}
          >
            {cat.label}
          </button>
      ...
      {activeTab === 'stay' && (
        <div className="space-y-3">
          <div className="bg-gray-50 border border-gray-200 rounded-xl px-4 py-4 flex items-center justify-between gap-4">
            ...
            <a href={`https://www.booking.com/country/${(currencyCode || 'us').toLowerCase().slice(0,2)}.html`} target="_blank" rel="noopener noreferrer" className="flex-shrink-0 text-xs font-medium px-4 py-2 rounded-full border border-gray-300 bg-white hover:bg-gray-50 transition-colors text-gray-900 whitespace-nowrap">Browse hotels →</a>
          </div>
          <div className="bg-gray-50 border border-gray-200 rounded-xl px-4 py-4 flex items-center justify-between gap-4">
            <p className="text-sm text-gray-700">🛏 Traveling on a budget? Hostels and guesthouses from around $15/night.</p>
            <a href="https://www.hostelworld.com" target="_blank" rel="noopener noreferrer" className="flex-shrink-0 text-xs font-medium px-4 py-2 rounded-full border border-gray-300 bg-white hover:bg-gray-50 transition-colors text-gray-900 whitespace-nowrap">Browse hostels →</a>
          </div>
        </div>
      )}
